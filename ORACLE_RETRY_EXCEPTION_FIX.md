...
### 🔍 根本原因
Spring Batch 的重试配置使用了过于宽泛的异常类型（`Exception.class`），导致 Spring Retry 机制无法正确注册和处理异常。`TerminatedRetryException` 表示重试机制无法识别异常类型。
...
#### 3️⃣ 配置说明
- **重试异常类型**: 只重试 `RuntimeException` 和 `DataAccessException`
- **跳过异常类型**: 只跳过 `RuntimeException` 和 `DataAccessException`
- **避免过于宽泛**: 不使用 `Exception.class`，避免重试机制混乱

...
/**
 * H2 数据库专用读取器
 * 针对 H2 数据库的 SQL 语法和特性优化
 */
@Component
public class H2Reader implements DataReader {
    ...
    public ItemReader<SourceRecord> createFullReader(int pageSize) {
        try {
            // 构建固定的 SQL 查询
            String selectClause = buildH2SelectClause();
            String fromClause = buildH2FromClause();
            String sortKey = "orders.id";

            System.out.println("========================================");
            System.out.println("创建 H2 全量分页读取器:");
            ...
            return new JdbcPagingItemReaderBuilder<SourceRecord>()
                    .name("h2FullPagingReader")
                    .dataSource(dataSource)
                    .queryProvider(queryProvider)
                    .pageSize(pageSize)
                    .rowMapper(new H2RecordRowMapper())
                    .build();

        } catch (Exception e) {
            System.err.println("创建 H2 全量读取器失败: " + e.getMessage());
            e.printStackTrace();
            throw new RuntimeException("Failed to create H2 full reader", e);
        }
    }
    ...
    public JdbcCursorItemReader<SourceRecord> createDeltaReader(LocalDateTime startTime, LocalDateTime endTime) {
        try {
            // 构建增量查询 SQL
            String sql = buildH2DeltaQuery(startTime, endTime);

            System.out.println("========================================");
            System.out.println("创建 H2 增量游标读取器:");
            System.out.println("  表名: " + tableName);
            ...
            System.err.println("创建 H2 增量读取器失败: " + e.getMessage());
            e.printStackTrace();
            throw new RuntimeException("Failed to create H2 delta reader", e);
        }
    }
     ...
     * H2 记录行映射器
     */
    private class H2RecordRowMapper implements RowMapper<SourceRecord> {
        @Override
    ...
    /**
     * 构建 H2 全量查询的 SELECT 子句
     */
    private String buildH2SelectClause() {
     ...
     * 构建 H2 全量查询的 FROM 子句
     */
    private String buildH2FromClause() {
     ...
     * 构建 H2 增量查询 SQL
     */
    private String buildH2DeltaQuery(LocalDateTime startTime, LocalDateTime endTime) {

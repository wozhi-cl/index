...
@TestPropertySource(properties = {
    ...
    "index.dataSource.csvPath=src/test/resources/test-data/sample-data.csv",
    "index.dataSource.csvFields=id,name,description,status,created_at,updated_at",
    "index.indexTarget.type=file",
    "index.indexTarget.path=./target/test-output",
    "index.indexTarget.fileName=csv-to-file-output.json"
})
public class CsvToFileJobTest extends AbstractJobTest {
    ...
    protected String getTestProfile() {
        return "test-csv-file";
    }

    @Override
    protected void prepareTestData() throws Exception {
        // CSV文件已经存在于 test-data/sample-data.csv
        System.out.println("✅ CSV测试数据文件已准备好");
        System.out.println("📁 文件路径: classpath:test-data/sample-data.csv");
        
        // 验证文件存在
        assertTrue(
            getClass().getClassLoader().getResource("test-data/sample-data.csv") != null,
            "CSV测试数据文件应该存在"
        );
    }
    ...
    protected void cleanupTestData() throws Exception {
        // 清理输出文件
        File outputFile = new File("./target/test-output/csv-to-file-output.json");
    ...
    @Test
    public void testFullIndexJobWithCsvToFile() throws Exception {
        System.out.println("\n" + "=".repeat(80));
        System.out.println("🧪 测试: 全量索引任务 - CSV → File");
        System.out.println("=".repeat(80));

        // 准备Job参数
        JobParameters jobParameters = new JobParametersBuilder()
            .addString("jobId", "csvToFile_test_" + System.currentTimeMillis())
        ...
        File[] outputFiles = outputDir.listFiles((dir, name) -> 
            name.startsWith("csv-to-file-output-") && name.endsWith(".json"));
    ...
    @Test
    public void testIncrementalIndexJobWithCsvToFile() throws Exception {
        System.out.println("\n" + "=".repeat(80));
        System.out.println("🧪 测试: 增量索引任务 - CSV → File");
        System.out.println("=".repeat(80));
        System.out.println("⚠️  注意: CSV数据源不支持增量模式，此测试预期会跳过或失败");

        // 准备Job参数
        JobParameters jobParameters = new JobParametersBuilder()
            .addString("jobId", "csvToFileIncr_test_" + System.currentTimeMillis())
            .addLong("timestamp", System.currentTimeMillis())
            .toJobParameters();

        // 尝试执行增量Job（CSV不支持增量，可能会失败）
        try {
            JobExecution jobExecution = jobLauncherTestUtils.launchJob(jobParameters);
            System.out.println("Job状态: " + jobExecution.getStatus());
            System.out.println("注意: CSV数据源通常不支持增量索引");
        } catch (Exception e) {
